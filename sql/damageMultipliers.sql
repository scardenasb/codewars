...
--
-- She will be using Grass-type Pokemon. Any fire pokemon you have will be strong against grass, 
-- but your water types will be weakened. The multipliers table within your Pokedex will take care of that.
--
...
SELECT p.pokemon_name, m.element, p.str*m.multiplier AS modifiedStrength
FROM pokemon p join multipliers m
ON p.element_id = m.id
WHERE p.str*m.multiplier >= 40 ORDER BY modifiedStrength DESC
